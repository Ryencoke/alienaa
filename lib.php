...
// Fabrication Lab ore tiers — level_req gates equipping (not crafting) a
// build made from that ore; also moved here so library.php can label each
// recipe's tier without duplicating this small mapping.
function weaponcraft_tiers(): array {
  return [
...
// Apartment districts + catalog (apartments.php) — moved here so library.php
// can list every apartment type too. Perk types are limited to what
// perk_apply() in apartments.php actually knows how to grant (cycles_max/
// signal_max/dual_boost applied directly to players; xp_bonus/foundry_bonus/
// bank_bonus read elsewhere via the apt_{key}:{pid} setting) — every type
// below reuses one of those six rather than inventing a new bonus category
// nothing else in the game would ever check for.
function apartment_catalog(): array {
  return [
    'districts' => [
...
// The Fabrication Lab catalog (weaponcraft.php) — moved here from being
// page-local so library.php can also list these homebrew builds (previously
// only Blacksmith items showed up in the Library at all). Row keys:
// id,name,type,atk,def,icon,tier,desc,cost. Deliberately distinct names
// from anything blacksmith_catalog() sells — see weaponcraft.php's own
// comment for why (one-off homebrew vs. catalog-bought gear).
function weaponcraft_recipes(): array {
  return [
    // ══════════ WEAPONS ══════════
...
// Renders a player's name as a profile link inside a notification body,
// colored/bolded for staff roles (matches the "username always gets
// role-color" rule elsewhere — chat_color($role,'') is role-only, never
// a member's own picked color). Used wherever a notification says a
// specific player did something to you (gave/loaned an item, sent
// credits) so the recipient can jump straight to that player's profile.
function notif_player_link(int $pid, string $username, ?string $role = null): string {
  $color = chat_color($role, '');
  $bold  = in_array($role, ['manager','admin','moderator','chatmod'], true) ? 'font-weight:700;' : '';
  return '<a href="index.php?p=profile&id=' . $pid . '" style="color:' . $color . ';' . $bold . 'text-decoration:none">' . e($username) . '</a>';
}
...
//
// Dedup is done with an explicit SELECT-before-INSERT here, not just
// INSERT IGNORE against the uq_pn_ref unique key. That key is added via a
// best-effort ALTER (ensure_player_notifications_table()) wrapped in a
// silent try/catch — on a host where it never actually took (e.g. it failed
// once against pre-existing duplicate rows before the key existed), INSERT
// IGNORE has nothing to collide with and happily inserts a fresh duplicate
// notification for the same still-unread message on every single poll tick.
// Dismissing the old row then does nothing to stop a new one appearing 12s
// later — which is exactly what "notifications keep reappearing" was.
function seed_player_notifications(PDO $pdo, int $pid): void {
  try {
    ...
    $mq->execute([$pid]);
    $exists = $pdo->prepare("SELECT 1 FROM player_notifications WHERE player_id=? AND ref_type='message' AND ref_id=?");
    $ins = $pdo->prepare("INSERT INTO player_notifications (player_id, type, body, ref_type, ref_id) VALUES (?, 'message', ?, 'message', ?)");
    foreach ($mq as $m) {
      $exists->execute([$pid, $m['id']]);
      if ($exists->fetchColumn()) continue;
      $ins->execute([$pid, "New message from " . $m['username'], $m['id']]);
    }
  } catch (Throwable $e) {}
  try {
    $tq = $pdo->prepare("SELECT t.id, p.id AS from_id, p.username, p.role, t.amount FROM tx_log t JOIN players p ON p.id=t.from_id WHERE t.to_id=? AND t.kind='transfer' AND t.created_at >= NOW()-INTERVAL 7 DAY ORDER BY t.created_at DESC LIMIT 10");
    $tq->execute([$pid]);
    $exists = $pdo->prepare("SELECT 1 FROM player_notifications WHERE player_id=? AND ref_type='tx' AND ref_id=?");
    $ins = $pdo->prepare("INSERT INTO player_notifications (player_id, type, body, ref_type, ref_id) VALUES (?, 'transfer', ?, 'tx', ?)");
    foreach ($tq as $t) {
      $exists->execute([$pid, $t['id']]);
      if ($exists->fetchColumn()) continue;
      $body = "+" . number_format((int)$t['amount']) . " credits received from " . notif_player_link((int)$t['from_id'], $t['username'], $t['role'] ?? null);
      $ins->execute([$pid, $body, $t['id']]);
    }
  } catch (Throwable $e) {}
}
